...
export default function BlueAgentTerminal({ logs, streamState }) {
  ...
  return (
    <div className="blue-terminal" ref={containerRef}>
      <div className="terminal-connection">
        WebSocket: {streamState || "connecting"}
      </div>
      {rows.length === 0 ? (
        <div className="terminal-line is-muted">No Blue-agent runtime output yet.</div>
      ) : (
